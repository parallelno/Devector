...
void dev::DevectorApp::SettingsInit()
{
	...
	AppStyleInit([this](const std::string& key, const std::string& def) {
		return GetSettingsString(key, def);
	});
...
void dev::DevectorApp::LoadRom(const std::string& _path)
{
	...
	m_hardwareP->Request(Hardware::Req::DEBUG_RESET, { {"resetRecorder", true}});
