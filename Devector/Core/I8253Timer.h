...
namespace dev
{
	...
	/*
	// A programmable interval timer based on Intel 8253 chip
	// https://en.wikipedia.org/wiki/Intel_8253
	//
	class Timer
	{
	private:
		uint16_t value[3]; // value of the counter
		uint16_t latched_value[3]; // latched value of the counter
		uint8_t control_word; // control word register
		uint8_t read_latch[3]; // read latch register
		uint8_t write_latch[3]; // write latch register
		bool bcd_mode[3]; // binary coded decimal mode
		bool mode[3]; // mode
	...
	public:
		...
		{
			if(index > 2 || (control_word & (1 << index)) == 0)
				return 0;
			uint16_t result = (latched_value[index] != -1) ? latched_value[index] : value[index];
			latched_value[index] = -1;
			if(bcd_mode[index])
				result = tobcd(result);
			return result;
		}
		...
		void set_control_word(uint8_t val)
		{
			control_word = val;
			for(int i = 0; i < 3; ++i)
			{
		...
		uint8_t get_control_word()
		{
			return control_word;
		}

		void set_bcd_mode(uint8_t index, bool val)
		{
			if(index > 2)
				return;
			bcd_mode[index] = val;
		...
		void set_mode(uint8_t index, bool val)
		{
			if(index > 2)
				return;
			mode[index] = val;
	...
	*/
	...
	public:
		...
		void write(int _addr, uint8_t _val)
		{
			_addr = ~(_addr & 3);
			switch (_addr) {
				case 0x03:
					return this->write_cw(_val);
				default:
					return this->counters[_addr].write_value(_val);
			}
		}
		...
		int read(int _addr)
		{
			...
			switch (_addr) {
				...
				default:
					return this->counters[_addr].read_value();
			}
		}
	...
	private:
		...
		int last_sound;
	public:
		TimerWrapper(I8253Timer & _timer) : timer(_timer),
