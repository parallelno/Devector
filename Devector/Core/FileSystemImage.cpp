...
void MDHeader::init_with_filename(std::wstring stem, std::wstring ext)
{
	...
	for (int i = 0, end = (int)dev::Min(sizeof(MDHeaderData::Name), stem.size());
			i < end; ++i) {
		fields->Name[i] = std::toupper(stem[i]);
	}

	for (int i = 0, end = (int)dev::Min(sizeof(MDHeaderData::Ext), ext.size());
			i < end; ++i) {
...
bool MDHeader::operator==(const MDHeader & rhs) const {
	return fields == rhs.fields ||
		...
		memcmp(fields, rhs.fields, sizeof(MDHeaderData)) == 0);
}
...
bool FileSystemImage::mount_local_dir(std::wstring path)
{
	tinydir_dir dir;
	...
	while (dir.has_next)
	{
		...
		std::wstring fullpath = path + L'/' + file.name;
		size_t length = dev::GetFileSize(fullpath);

		//printf("%s, %lx\n", file.name, length);

		if (!file.is_dir && length <= MAX_FS_BYTES) {
			auto result = dev::LoadFile(fullpath);
			if (result && result->size() > 0) {
				/* auto [ok, filename] =*/ dev::SaveFile(file.name, *result);
			}
		}
...
Dirent FileSystemImage::find_file(const std::wstring& filename)
{
	std::vector<uint8_t> protobuf(MDHeader::SIZE);
	MDHeader proto(&protobuf[0]);
	auto [_, stem, ext] = dev::GetDirStemExt(filename);
	if (ext.size() > 0) {
		ext = ext.substr(1);
	}
...
FileSystemImage::bytes_t FileSystemImage::read_bytes(const Dirent & de)
{
	...
	for (uint16_t c : de.chain) {
		...
		for (int i = 0; i < 2; ++i) {
			...
			int tocopy = dev::Min(SECTOR_SIZE, remainder);
...
FileSystemImage::bytes_t FileSystemImage::read_file(const std::wstring & filename)
{
...
auto FileSystemImage::allocate_file(const std::wstring & filename,
		const bytes_t& content, const chain_t& chain)
-> std::wstring
{
	auto [stem, ext] = unique_cpm_filename(filename);
	std::wstring internal_filename = stem + L"." + ext;
	...
	for (auto h = begin(); h != end() && remaining > 0; ++h) {
		if ((*h).user() >= 0x10) {
			proto.fields->Records = dev::Min(0x80, (remaining + 127) / 128);
...
-> std::tuple<bool,std::wstring>
{
	...
	for (auto c : chain) {
		...
		for (int i = 0; i < 2; ++i) {
			...
			int tocopy = dev::Min(SECTOR_SIZE, remaining);
...
auto FileSystemImage::unique_cpm_filename(const std::wstring & filename)
-> std::tuple<std::wstring, std::wstring>
{
	auto [_, stem, ext] = dev::GetDirStemExt(filename);


	constexpr size_t name_sz = sizeof(MDHeader::MDHeaderData::Name);
	constexpr size_t ext_sz = sizeof(MDHeader::MDHeaderData::Ext);
	dev::StrToUpper(stem);
	dev::StrToUpper(ext);
	ext = ext.substr(1, ext_sz);
	stem = stem.substr(0, name_sz);

	auto ReplaceDotWithDash = [](std::wstring _str) {
		for (auto& c : _str) {
			if (c == '.') c = '_';
		}
	};
	ReplaceDotWithDash(stem);
	ReplaceDotWithDash(ext);

	bool name_taken = taken_names.count(stem + L"." + ext);
	int n = 1;
	while (name_taken) {
		int nc = std::to_string(n).size() + 1;
		stem = stem.substr(0, name_sz - nc);
		stem += L"@" + std::to_wstring(n);
		++n;
		name_taken = taken_names.count(stem + L"." + ext);
	}

	return {stem, ext};
}

auto MDHeader::name() const 
-> std::wstring
{
	return dev::TrimSpacesRight(std::wstring(fields->Name, 8));
}

auto MDHeader::ext() const
-> std::wstring
{
	return dev::TrimSpacesRight(std::wstring(fields->Ext, 3));
}