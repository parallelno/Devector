...
dev::Sound::Sound(TimerWrapper& tw/*, AYWrapper& aw*/) :
    timerwrapper(tw)/*, aywrapper(aw)*/
{
    init();
}

void dev::Sound::init(/*WavRecorder* _rec*/)
{
    //this->rec = _rec;
    ...
    //want.callback = dev::Sound::callback;
...
void dev::Sound::soundSteps(int nclk1m5, int tapeout, int covox, int tapein)
{
    covox = covox - 255;
    /*
...
void dev::Sound::pause(int pause)
{
    //if (!Options.nosound) 
    {
...
void dev::Sound::callback(void* userdata, uint8_t* stream, int len)
{
    Sound* that = (Sound*)userdata;
    ...
    else
    {
        ...
        if (++that->rdbuf == dev::Sound::NBUFFERS) {
            that->rdbuf = 0;
        }
    }
...
void dev::Sound::sample(float samp)
{
    //if (!Options.nosound) 
    {
        ...
        if (this->wrptr >= this->sound_frame_size * 2) {
            this->wrptr = 0;
            if (++this->wrbuf == dev::Sound::NBUFFERS) {
                this->wrbuf = 0;
            }
        }
