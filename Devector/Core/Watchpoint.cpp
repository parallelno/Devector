...
->const bool
{
	...
	else if (m_globalAddr + 1 != _globalAddr) {
		return false;
	} 
	else{
		break_p = &m_breakH;
...
auto dev::Watchpoint::IsActive() const -> bool { return m_active; }
auto dev::Watchpoint::GetGlobalAddr() const -> GlobalAddr {	return m_globalAddr; }
auto dev::Watchpoint::GetAccess() const -> Access {	return m_access; }
...
auto dev::Watchpoint::GetLen() const -> int { return m_len; }
auto dev::Watchpoint::GetComment() const -> const std::string& { return m_comment; }
auto dev::Watchpoint::GetId() const -> Id {	return m_id;}
...
void dev::Watchpoint::Print() const
{
	std::printf("0x%05x, access: %s, cond: %s, value: 0x%04x, type: %s, len: %d, active: %d \n", 
		m_globalAddr, GetAccessS(), GetConditionS(),
		m_value, GetTypeS(), m_len, m_active);
}
